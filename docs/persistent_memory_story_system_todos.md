...
- [ ] Add `story_worlds` table to `data/schema.sql`:
...
- [ ] Add `characters` table to `data/schema.sql`:
...
- [ ] Add `narrative_threads` table to `data/schema.sql`:
...
- [ ] Add `episodes` table to `data/schema.sql`:
...
- [ ] Implement `data/dao_story_worlds.py` — CRUD for story_worlds: `insert_world()`, `get_world()`, `update_world()`, `increment_episode_number()`
- [ ] Implement `data/dao_characters.py` — CRUD for characters: `insert_character()`, `get_characters_for_world()`, `update_character()`, `update_arc_stage()`
- [ ] Implement `data/dao_threads.py` — CRUD for narrative_threads: `insert_thread()`, `get_open_threads()`, `get_threads_for_world()`, `resolve_thread()`, `add_escalation_point()`
- [ ] Implement `data/dao_episodes.py` — CRUD for episodes: `insert_episode()`, `get_episodes_for_world()`, `get_latest_episode()`, `update_episode_status()`

### S0.3 Phase S0 Tests

- [ ] DAO round-trip tests for each new DAO module (insert, read, update)
- [ ] Schema migration test: verify `get_initialized_connection()` creates new tables alongside existing ones
- [ ] Test audience_profile_json serialization/deserialization
- [ ] Test thread lifecycle: open -> escalating -> climax -> resolved
- [ ] Test character arc_stage transitions: introduction -> rising -> crisis -> resolution -> transformed
...
- [ ] Implement `agents/story_memory_loader_agent.py`
...
- [ ] Implement `agents/premise_architect_agent.py`
...
- [ ] Implement `agents/plot_architect_agent.py`
...
- [ ] Implement `agents/scene_writer_agent.py`
...
- [ ] Implement `agents/canon_updater_agent.py`
...
- [ ] Implement `agents/audience_compliance_agent.py`
...
- [ ] Implement `agents/narration_formatter_agent.py`
...
- [ ] Unit tests for each new agent's parse/validate with sample JSON responses
- [ ] StoryMemoryLoaderAgent test: loads from pre-populated DB, produces correct state keys
- [ ] PremiseArchitectAgent: validates thread selection, rejects empty premise
- [ ] PlotArchitectAgent: validates scene count bounds, POV character existence
- [ ] SceneWriterAgent: validates all scene_ids present in output
- [ ] CanonUpdaterAgent: validates claim citations, claim_type values
- [ ] AudienceComplianceAgent: validates PASS/FAIL output, violation format
- [ ] NarrationFormatterAgent: validates non-empty outputs
...
- [ ] Write `graphs/story_graph.yaml` — 11-node chain:
...
- [ ] Extend `agents/qa_validator_agent.py` with story-domain gate rules:
...
- [ ] Implement `scripts/run_story.py` — CLI entrypoint:
...
- [ ] Create `examples/story_seed.json` — minimal world config:
...
- [ ] Integration test: multi-episode continuity
  ...
  - Verify: character arc_stage progressed
  - Verify: thread escalation tracked
- [ ] QA gate tests: each story-domain rule (pass and fail cases)
...
- [ ] Extend `agents/publisher_agent.py` to handle `scope_type = "story"`:
...
- [ ] Extend `publish/renderer.py` with story templates:
...
- [ ] Implement episode-number versioning in `publisher_agent.py`:
...
- [ ] Golden test: story publish output matches expected artifact structure
- [ ] Manifest.json schema validation for story publishes
- [ ] Artifacts.json integrity: all listed paths exist, hashes match
- [ ] Episode.md readable format test
- [ ] Multi-episode publish: E001 and E002 in separate version directories
...
- [ ] Implement arc_stage transition enforcement in dao_characters:
...
- [ ] Test arc_stage transition validation (valid and invalid)
